...
M15 release-validation records retained by the repository:
...
- M16 SystemVerilog artifact-boundary validation;
- Verilator parsing and module elaboration;
- executable M16 RTL testbench generation;
- M16 architectural simulation;
- M16 assertion execution;
...
- M16 RTL qualification artifact generation;
...
- FPGA active-neutral first-leg execution;
- FPGA retained pending-route completion;
...
`M16 SystemVerilog RTL core realization`
...
`Verilator parsing, module elaboration, executable testbench generation, simulation, and assertion execution`
...
`FPGA scheduler, request-interface, active-neutral route, and pending-route propagation`
...
The FPGA integration boundary preserves:
...
The core dynamic relations are executed by the qualified M15 Python semantic reference.
...
Qualified default fractal coupling exponent:
...
| Ternary state | RTL symbol | Encoding |
|---|---|---|
...
M15 phase-derived ternary target mapping:
...
M15 default 16-cell request-lane count:

`4`

M15 validated relation:

`switch_load_peak <= transition_fraction`

Qualified M16 RTL testbench profile:
...
Qualified RTL invariant result:
...
The badges link directly to their workflow pages and obtain their displayed state from the corresponding GitHub Actions badge endpoints.
...
The RTL workflow qualifies the executable SystemVerilog retained-state execution boundary.
...
The workflow validates exactly five RTL documentation files:
...
The artifact-boundary stage rejects:

- missing required SystemVerilog files;
- unexpected SystemVerilog files in `rtl/m16`;
- missing required RTL documentation files;
- empty required RTL artifacts.

The workflow also requires the absence of:
...
The workflow records SHA-256 hashes for:
...
| File | Qualified condition |
|---|---|
| `rtl/m16/frp_m16_state_update.sv` | complete combinational assignment and zero inferred latches |
| `rtl/m16/frp_m16_request_lanes.sv` | complete combinational assignment and zero inferred latches |
...
It also requires an empty `git status --porcelain` result and the absence of repository-local build directories:
...
`frp-m16-fpga-preparation-${{ github.ref }}`
...
The closed FPGA preparation documentation contains:

1. `fpga/m16/SIMULATION_TRANSCRIPT.md`;
2. `fpga/m16/CLOSURE.md`.

The workflow validates nine qualified RTL dependencies:
...
1. `Checkout repository`;
2. `Verify M16 FPGA artifact boundary`;
3. `Prepare isolated FPGA simulation paths`;
4. `Install Verilator toolchain`;
5. `Record M16 FPGA and RTL source hashes`;
6. `Elaborate M16 FPGA integration top`;
7. `Build M16 FPGA integration testbench`;
8. `Reject latch and multidriven diagnostics`;
9. `Execute M16 FPGA integration testbench`;
10. `Validate FPGA integration terminal markers`;
11. `Record FPGA preparation qualification result`;
12. `Verify repository remains unchanged`;
13. `Upload M16 FPGA qualification evidence`;
14. `Publish FPGA preparation summary`.
...
2. the first rising clock edge after release sets synchronization stage `0`;
3. the second rising clock edge sets synchronization stage `1`;
...
- `git diff --exit-code`;
- an empty `git status --porcelain` result;
- no `obj_dir`;
- no `rtl/m16/obj_dir`;
- no `fpga/m16/obj_dir`.
...
The qualified M15 workflow executes nine named stages:
...
## 12. M15 Structured Output Generation
...
The qualified M15 workflow runs four self-test variants.
...
## 14. Exact Qualified M15 41-Check Coverage

The qualified M15 self-test package contains exactly these 41 checks:
...
## 21. Cycle-Exact Trace Validation
...
The qualified M15 export defines exactly 26 tick-execution stages:
...
The qualified M15 assertion harness contains exactly 13 assertions:
...
The qualified M15 workflow validates execution at:
...
`Verilator parsing, elaboration, executable RTL simulation, and assertion execution`
...
## 43. Full Local M15 CI Reproduction Sequence
...
M16 FPGA preparation evidence records:

- `fpga/m16/SIMULATION_TRANSCRIPT.md`;
- `fpga/m16/CLOSURE.md`.

Current M16 event and invariant record:
...
Recorded M15 validation result:
...
`executable M16 RTL architectural testbench`
...
`assertion execution and ten integrated invariant flags`
...
`scheduler, request-interface, active-neutral route, and pending-route propagation`
...
`all ten FPGA-boundary invariant flags`
