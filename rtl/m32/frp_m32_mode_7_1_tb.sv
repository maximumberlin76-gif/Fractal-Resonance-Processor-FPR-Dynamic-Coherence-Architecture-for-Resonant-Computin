...
// FRP M32 deterministic registered-target 7/1 scheduler testbench.

`ifndef FRP_M32_MODE_7_1_TB_SV
`define FRP_M32_MODE_7_1_TB_SV
...
module frp_m32_mode_7_1_tb;
  ...
  task automatic start_tick(
    ...
    begin
      ...
      if (scheduler_state_q !== expected_state) begin
        $fatal(
          1,
          "FRP M32 7/1 TB: scheduler mismatch expected=%0d actual=%0d",
          expected_state,
          scheduler_state_q
        );
      end
  ...
  initial begin : qualification_sequence
    ...
    if (scheduler_mode_q !== FRP_MODE_7_1)
      $fatal(1, "FRP M32 7/1 TB: scheduler mode registration mismatch");
    if (scheduler_state_q !== FRP_SCHED_BALANCE)
      $fatal(1, "FRP M32 7/1 TB: initial balance state mismatch");
    if (state_out !== {CELLS{FRP_ACTIVE_NEUTRAL}})
      $fatal(1, "FRP M32 7/1 TB: reset state is not active zero");
    if (pending_route_out !== {CELLS{FRP_ACTIVE_NEUTRAL}})
      $fatal(1, "FRP M32 7/1 TB: reset pending-route word mismatch");
    if (registered_target_valid_q)
      $fatal(1, "FRP M32 7/1 TB: reset target validity mismatch");
    ...
    if (cell_s32(phase_projection_q30, 0) !== FRP_M31_Q30_ONE)
      $fatal(1, "FRP M32 7/1 TB: positive phase projection mismatch");
    if (cell_state(phase_target_source, 0) !== FRP_STATE_POS)
      $fatal(1, "FRP M32 7/1 TB: positive source target mismatch");
    if (!phase_target_domain_valid || !registered_target_domain_valid)
      $fatal(1, "FRP M32 7/1 TB: target domain mismatch");
    ...
    for (int tick = 0; tick < 16; tick++) begin
      ...
      if (tick == 8) begin
        ...
        if (cell_state(phase_target_source, 0) !== FRP_STATE_NEG)
          $fatal(1, "FRP M32 7/1 TB: opposite source target mismatch");
        if (cell_state(registered_target_q, 0) !== FRP_STATE_POS)
          $fatal(1, "FRP M32 7/1 TB: source bypassed target register");
        if (cell_state(execution_target_bank, 0) !== FRP_STATE_POS)
          $fatal(1, "FRP M32 7/1 TB: source bypassed execution target");
      end

      start_tick(expected_scheduler);

      if (!target_capture_accepted || target_capture_rejected)
        $fatal(1, "FRP M32 7/1 TB: target capture mismatch");
      if (execution_target_bank !== registered_target_q)
        $fatal(1, "FRP M32 7/1 TB: execution target registration mismatch");
      ...
      if (tick < 7) begin
        if (cell_state(state_out, 0) !== FRP_ACTIVE_NEUTRAL)
          $fatal(1, "FRP M32 7/1 TB: balance changed active zero");
        if (cell_state(pending_route_out, 0) !== FRP_STATE_ZERO)
          $fatal(1, "FRP M32 7/1 TB: balance created pending route");
      end else if (tick == 7) begin
        if (cell_state(state_out, 0) !== FRP_STATE_POS)
          $fatal(1, "FRP M32 7/1 TB: commit did not release positive state");
      end else if (tick == 8) begin
        if (cell_state(registered_target_q, 0) !== FRP_STATE_NEG)
          $fatal(1, "FRP M32 7/1 TB: opposite target was not registered");
        if (cell_state(state_out, 0) !== FRP_STATE_POS)
          $fatal(1, "FRP M32 7/1 TB: capture tick changed retained state");
      end else if (tick < 15) begin
        if (cell_state(state_out, 0) !== FRP_ACTIVE_NEUTRAL)
          $fatal(1, "FRP M32 7/1 TB: first route leg state mismatch");
        if (cell_state(pending_route_out, 0) !== FRP_STATE_NEG)
          $fatal(1, "FRP M32 7/1 TB: pending polarity was not retained");
      end else begin
        if (!second_leg_before_tick)
          $fatal(1, "FRP M32 7/1 TB: second route leg was not accepted");
        if (cell_state(state_out, 0) !== FRP_STATE_NEG)
          $fatal(1, "FRP M32 7/1 TB: second route leg did not complete");
        if (cell_state(pending_route_out, 0) !== FRP_STATE_ZERO)
          $fatal(1, "FRP M32 7/1 TB: completed pending route did not clear");
      end

      if (actual_direct_events !== '0)
        $fatal(1, "FRP M32 7/1 TB: direct execution event detected");
      if (reserved_state_events !== '0)
        $fatal(1, "FRP M32 7/1 TB: reserved state event detected");
      if (queue_overflow_events !== '0)
        $fatal(1, "FRP M32 7/1 TB: pending-route overflow detected");
      if (invariant_flags !== {FRP_M31_INVARIANT_FLAGS{1'b1}})
        $fatal(1, "FRP M32 7/1 TB: inherited invariant failed");

      $write(
        "M32_MODE_7_1_TRACE tick=%0d scheduler_state=%0d ",
        tick,
        expected_scheduler
      );
      $write(
        "balance_tick=%0d commit_tick=%0d ",
        expected_scheduler == FRP_SCHED_BALANCE,
        expected_scheduler == FRP_SCHED_COMMIT
      );
      $write(
    ...
    if (accepted_target_capture_events_q !== 16)
      $fatal(1, "FRP M32 7/1 TB: accepted capture count mismatch");
    if (rejected_target_capture_events_q !== '0)
      $fatal(1, "FRP M32 7/1 TB: rejected capture count mismatch");
    if (ticks_recorded_q !== 16)
      $fatal(1, "FRP M32 7/1 TB: tick count mismatch");
    if ((scheduler_count_balance_q !== 14)
        || (scheduler_count_commit_q !== 2)
        || (scheduler_count_free_q !== '0)
        || (scheduler_count_excite_q !== '0)
        || (scheduler_count_neutralize_q !== '0)) begin
      $fatal(1, "FRP M32 7/1 TB: scheduler cadence count mismatch");
    end
    if (thermal_sample_count_q !== 16)
      $fatal(1, "FRP M32 7/1 TB: thermal sample count mismatch");
    if (temperature_proxy_q16 <= 0)
      $fatal(1, "FRP M32 7/1 TB: thermal proxy did not integrate activity");
    if (cell_s32(frequency_current_q16, 0)
        === FRP_M31_BASE_FREQUENCY_Q16) begin
      $fatal(1, "FRP M32 7/1 TB: retained frequency did not evolve");
    end
    if (!stable)
      $fatal(1, "FRP M32 7/1 TB: stability state mismatch");

    $display("FRP_M32_REGISTERED_TARGET_MODE_7_1_TB: PASS");
    $finish;
  end
